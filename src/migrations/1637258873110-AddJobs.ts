...
export class AddJobs1637258873110 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
      ...
      const businessFields = await businessFieldRepository.createQueryBuilder('businessField')
          .where("businessField.name != 'Information Technology'").getMany();
      ...
      for(const rawJob of rawJobs) {
        ...
        job.skills = skills.sort(() => .5 - Math.random()).slice(0, 3);
